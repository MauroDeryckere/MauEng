...
namespace MauCor
{
	void InternalAssert(LogCategory const& category, char const* expression, char const* file, int line, char const* message) noexcept
	{
		ME_LOG_FATAL(category, "Assertion failed: {} File: {}, Line: {}, Message: {}", expression, file, line, message ? message : "NO MESSAGE SPECIFIED");
	}

	void InternalCheck(LogCategory const& category, char const* expression, char const* file, int line, char const* message) noexcept
	{
		ME_LOG_FATAL(category, "Check failed: {} File: {}, Line: {}, Message: {}", expression, file, line, message ? message : "NO MESSAGE SPECIFIED");
	}

	void InternalVerify(LogCategory const& category, char const* expression, char const* file, int line, char const* message) noexcept
	{

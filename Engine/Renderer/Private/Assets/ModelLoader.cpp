...
namespace MauRen
{
	LoadedModel ModelLoader::LoadModel(std::string const& path, VulkanCommandPoolManager& cmdPoolManager, VulkanDescriptorContext& descriptorContext) noexcept
	{
		...
		{
			ME_LOG_ERROR(MauCor::ELogCategory::Renderer, "Error loading model! {}", importer.GetErrorString());
			return model;
		}
		...
		std::string const& path)
	{
		...
		for (unsigned j{ 0 }; j < mesh->mNumVertices; ++j)
		{
			...
			{
				ME_LOG_WARN(MauCor::ELogCategory::Renderer, "Missing texcoords for vertex {}", j);
			}
		...
		if (AI_SUCCESS != result)
		{
			...
			ME_LOG_WARN(MauCor::ELogCategory::Renderer, "Setting material name manually for: {}", matStr);
		}

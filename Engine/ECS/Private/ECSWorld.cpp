...
namespace MauEng::ECS
{
	...
	ComponentType& ECSWorld::GetComponent(EntityID id) const noexcept
	{
		ME_ASSERT(HasComponent<ComponentType>(id));
		return m_pImpl->GetComponent<ComponentType>(id);
	}
	template <typename ComponentType>
	ComponentType& ECSWorld::GetComponent(Entity const entity) const noexcept
	{
		return GetComponent<ComponentType>(entity.ID());
	}

	template <typename ComponentType>
	void ECSWorld::RemoveComponent(EntityID id) noexcept
	{
		ME_ASSERT(HasComponent<ComponentType>(id));
		m_pImpl->RemoveComponent<ComponentType>(id);
	}
	template <typename ComponentType>
	void ECSWorld::RemoveComponent(Entity const entity) noexcept
	{
		RemoveComponent<ComponentType>(entity.ID());
	}
